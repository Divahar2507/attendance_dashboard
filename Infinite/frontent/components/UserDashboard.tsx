...
import { User, Ticket, TicketStatus, TicketUpdate } from '../types';
import {
  CheckCircle2,
  Clock,
  FileText,
...
const UserDashboard: React.FC<UserDashboardProps> = ({ user }) => {
  ...
  const myTickets = useMemo(() => allTickets.filter(t => t.userId === user.id), [allTickets, user.id]);
  const poolTickets = useMemo(() => allTickets.filter(t => !t.userId), [allTickets]);
  ...
  const handleSelfAssign = async (ticketId: number) => {
    try {
      const response = await fetch(`${API_BASE_URL}/tickets/${ticketId}`, {
        ...
        body: JSON.stringify({ userId: user.id, status: TicketStatus.OPEN }),
      });
